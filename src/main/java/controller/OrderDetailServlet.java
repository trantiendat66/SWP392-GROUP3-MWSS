...
public class OrderDetailServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            out.println("<p><strong>Customer:</strong> " + order.getCustomer_name() + "</p>");
            out.println("<p><strong>Date:</strong> " + order.getOrder_date() + "</p>");
            out.println("<p><strong>Phone:</strong> " + order.getPhone() + "</p>");
            out.println("<p><strong>Address:</strong> " + order.getShipping_address() + "</p>");
            out.println("<p><strong>Status:</strong> " + order.getOrder_status() + "</p>");
            out.println("<p><strong>Total:</strong><span style='color:green;font-weight:bold;'> " + order.getTotal_amount() + " VND</p>");
            out.println("</div>");
            out.println("</div>");
            ...
            for (OrderDetail od : orderP) {
                ...
                out.println("<img src='" + request.getContextPath() + "/assert/image/" + od.getImage()
                        + "' alt='" + od.getProduct_name() + "' style='width:200px;height:auto;margin:5px;'/>");
                out.println("<div><strong>Product: </strong>" + od.getProduct_name() + "</div>");
                out.println("<div><strong>Quantity: </strong>" + od.getQuantity() + "</div>");
                out.println("<div><strong>Price: </strong>" + od.getUnit_price() + " VNĐ</div>");
                out.println("<div><strong>Total: </strong><span style='color:#007bff;'>" + od.getTotal_price() + " VNĐ</div>");
                out.println("</div>");
            }
            out.println("</div>");
            out.println("</div>");
